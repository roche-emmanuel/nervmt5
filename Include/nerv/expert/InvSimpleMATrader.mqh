...
class InvSimpleMATrader : public nvPeriodTrader
{
protected:
...
public:
  InvSimpleMATrader(const nvSecurity& sec, ENUM_TIMEFRAMES period) : nvPeriodTrader(sec,period)
  {
  ...
  void handleBar()
  {
    ...
    if(selectPosition())
    {
      ...
      tp = isBuy ? MathMax(tp,takeprofit) : MathMin(tp,takeprofit);

      // Also update the take profit at 2x the diff between the ma4 and ma20
      double nsl = isBuy ? prev_ema4 - (prev_ema-prev_ema4) : prev_ema4 + (prev_ema4 - prev_ema);
      ...
      && delta0>1.1*delta1)
    {
      ...
      double sl = prev_ema4 + prev_ema4 - prev_ema;
      double tp = prev_ema;
      ...
      && delta0<1.1*delta1)
    {
      ...
      double sl = prev_ema4 - (prev_ema-prev_ema4);
      double tp = prev_ema;
