...
local function make_assertion(name, message, func)
  ...
  if type(message) == "function" then
    ...
    format_message = function(message, ...)
      ...
      for i = 1, nargs do 
        a[i] = type(args[i])=="string" and '"'.. args[i] ..'"' or tostring(args[i]) 
      end
...
make_assertion("error",        "result of %s to be an error",       function(f) return not pcall(f) end)
...
make_assertion("truthy",       "%s to be truthy",                        function(a) return (a ~= false and a ~= nil) end)
make_assertion("type",         "%s to be a %s",                          function(a, b) return type(a) == b end)
make_assertion("close",        "%s to be close to %s (eps=%s)",          function(a, b, c) return math.abs(a - b) <= c end)
...
make_assertion("not_error",    "result of %s not to be an error",   function(f) return not not pcall(f) end)
...
local function load_contexts(target, contexts)
  ...
  for _, v in ipairs(context_aliases) do 
    env[v] = context_block
    env["x"..v] = noop 
  end
  for _, v in ipairs(test_aliases)    do 
    env[v] = test_block 
    env["x"..v] = noop
  end
